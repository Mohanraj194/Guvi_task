...
(<HTMLButtonElement>document.getElementById('deposit')).addEventListener('click',() => {
    ...
    if (gCheck){ 
        let depositInvoice = new Invoice(invoiceArrList.length+1,customer,amount,details);
...
(<HTMLButtonElement>document.getElementById('withdraw')).addEventListener('click',() => {
    let customerID = + (<HTMLSelectElement>document.getElementById('walletCustomerList')).value;
    let amount = +(<HTMLInputElement>document.getElementById('walletAmount')).value;
    if(customerID !== NaN && amount != 0) {
    ...
    let gCheck = (<HTMLInputElement>document.getElementById('generateInvoice')).checked;
    if(gCheck && wCheck){
        let withdrawlInvoice = new Invoice(invoiceArrList.length+1,customer,amount,details);
...
(<HTMLButtonElement>document.getElementById('purchase-btn')).addEventListener('click',() => {
    ...
    let invoice = new Invoice(invoiceArrList.length+1,customerAccount.getCustomer(),amount,details);
