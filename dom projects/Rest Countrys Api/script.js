...
function appendData(){
    ...
    fetchData.map(element=>{
                ...
                card_h4.innerText = element['name'].toUpperCase() 

                let card_p = document.createElement("p")
                card_p.setAttribute("class","card-text")
                card_p.innerHTML=`<b>Captial : </b> ${element['capital'].toUpperCase() }<br/> <b>Population : </b> ${element['population']} <br/> <b>Area : </b> ${element['area']} <br/> <b>Region : </b> ${element['region']}`
...
function search(){
    ...
    val = val.toLowerCase();
    let res = fetchData.filter((element)=>element['name'].toLowerCase().includes(val))

    document.getElementById("main_div").innerHTML=``
    let h3 = document.createElement("h3")
    h3.setAttribute("class","text-center w-100 p-2")
   if (val.length)h3.innerHTML="Search Result" 
   else h3.innerHTML="Country Details"
