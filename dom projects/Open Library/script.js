...
open_lib_logo.src="img/logo.svg"
...
async function getData(){
    ...
    try {
        ...
        let result =  data['docs'].filter((element)=>element['has_fulltext']==true)
        console.log(result)
        if(result.length>0){
            ...
            for(i=0; i < result.length; i++){
                ...
                card_img.setAttribute("src",'https://covers.openlibrary.org/b/id/'+result[i]['cover_i']+'-L.jpg');
                ...
                card_p.innerHTML=`<b>Author Name:</b> ${result[i]['author_name'][0]}<br> <b>publish_year : </b> ${result[i]['publish_year'][0]} <br> <b>Subject : </b> ${result[i]['subject'].join()} `
